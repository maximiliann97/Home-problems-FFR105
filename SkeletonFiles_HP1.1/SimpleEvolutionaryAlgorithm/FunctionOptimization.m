...
for k = 1:limit
zValues(j,k) = ...
EvaluateIndividual([xValues(j,k) yValues(j,k)]);
end
end
...
for i = 1:2:populationSize
    ...
    if (r < crossoverProbability)
        ...
        tempPopulation(1+i,:) = newChromosomePair(2,:);
    else
...
set(populationPlotHandle,'XData',decodedPopulation(:,1),...
'YData', decodedPopulation(:,2),'ZData',fitness(:));
drawnow;
